...
void renderCPUFrame(uchar3* frameBuffer, camera c, 
    ...
    //spawn threads
    std::vector<renderCPURowArgs> threadArgs(numThreads);
    int counter = 0;
    std::ranges::generate(threadArgs, [args_template, &counter]() {
        auto temp = args_template;
        temp.rowOffset = counter++;
        return temp;
    });
    ...
    threadArgs.begin(),
    threadArgs.end(),
    [](const renderCPURowArgs& args) {
            renderCPURows((void*)&args);
        }
    );
}
